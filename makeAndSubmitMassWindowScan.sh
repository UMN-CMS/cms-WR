...
do
	...
	#eval "bsub -R 'pool>3000' -q 8nh -J scanMassWindows_ee_MWR_${mass[$i]} < runMassWindowScan_MWR_${mass[$i]}_ee.sh"
	eval "bsub -R 'pool>3000' -q 8nh -J scanMassWindows_mumu_MWR_${mass[$i]} < runMassWindowScan_MWR_${mass[$i]}_mumu.sh"
