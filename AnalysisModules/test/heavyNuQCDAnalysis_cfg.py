...
if isData:
    if runMuonAnalysis or runElectronAnalysis:
...
if isPFJets:
    ...
    if isMC:
        usePF2PAT_WREdition(process,runPF2PAT=True, jetAlgo='AK5', runOnMC=isMC, postfix=postfix,
                            ...
                            pvCollection=cms.InputTag('goodOfflinePrimaryVertices'),typeIMetCorrections=False)
    else:
        usePF2PAT_WREdition(process,runPF2PAT=True, jetAlgo='AK5', runOnMC=isMC, postfix=postfix,
...
if isData:
    if (runMuonAnalysis or topStudy) and runElectronAnalysis:
        ...
        if doDijet:
            triggerMatchersList = [ 'muonTriggerMatchHLTMuons','qcdMuonTriggerMatchHLTMuons' ]
    ...
    if doDijet:
        switchOnTriggerMatchEmbedding( process, triggerMatchers = [ 'qcdMuonTriggerMatchHLTMuons' ], outputModule = '' )
        removeCleaningFromTriggerMatching( process, outputModule = '' )
        # if runEra == "2012AB" or "2012Cr" or "2012Cp" or "2012D":
        process.qcdMuonTriggerMatchHLTMuons.matchedCuts = cms.string( 'path( "HLT_Mu40_v*",1,0 )' )
...
process.hNuQCD.trigMatchPset.muTriggerEta     = cms.double( 2.4 )
...
process.hNuQCDMu40.trigMatchPset.muTriggerEta       = cms.double( 2.4 )
process.hNuQCDMu40eta2p1.trigMatchPset.triggerPt    = cms.double( 40. )
process.hNuQCDMu40eta2p1.trigMatchPset.muTriggerEta = cms.double( 2.1 )
...
process.hNuQCDMu40Loose       = process.hNuQCDMu40.clone()
process.hNuQCDMu40eta2p1Loose = process.hNuQCDMu40eta2p1.clone()
process.hNuQCDMu40Loose.muIDPset.eraForId       = cms.int32(-1)
process.hNuQCDMu40eta2p1Loose.muIDPset.eraForId = cms.int32(-1)
...
if doDijet:
    ...
    process.pDijet    = cms.Path( process.AnalysisIntroSequence + process.producePFMETCorrections + process.hNuQCDMu40 )
    process.pDijet2p1 = cms.Path( process.AnalysisIntroSequence + process.producePFMETCorrections + process.hNuQCDMu40eta2p1 )
    process.pDijetLoose    = cms.Path( process.AnalysisIntroSequence + process.producePFMETCorrections + process.hNuQCDMu40Loose )
    process.pDijet2p1Loose = cms.Path( process.AnalysisIntroSequence + process.producePFMETCorrections + process.hNuQCDMu40eta2p1Loose )
