...
void quickSignalZpt()
{
	...
	chain_DYMadInclEE->Add(dir+"selected_tree_DYMAD_signal_eeEE"+mcFileTag+".root");
	chain_DYAmcInclEE->Add(dir+"selected_tree_DYAMC_signal_eeEE"+mcFileTag+".root");
	//chain_dataEE->Add(dir+"selected_tree_data_lowdileptonsidebandEE"+dataFileTag+".root");
	...
	chain_DYMadInclMuMu->Add(dir+"selected_tree_DYMAD_signal_mumuMuMu"+mcFileTag+".root");
	chain_DYAmcInclMuMu->Add(dir+"selected_tree_DYAMC_signal_mumuMuMu"+mcFileTag+".root");
	//chain_dataMuMu->Add(dir+"selected_tree_data_lowdileptonsidebandMuMu"+dataFileTag+".root");
	...
	Float_t minLeadLeptonPt = 60, minSubleadLeptonPt = 50, maxMll = 13000, minMll = 200, maxLeptonEta = 2.4;
...
void MakeHistos(TChain * chain, Selector *myEvent, std::vector<TH1F*> *hs, Float_t leadLeptonPtCut, Float_t subleadLeptonPtCut, Float_t upperMllCut, Float_t lowerMllCut, Float_t leptonEtaCut, Float_t normRescale)
{
	...
	TH1F *h_Z_pt = new TH1F("h_Z_pt", "", 80, -50., 800.);
...
void drawPlots(TH1F* hs_DYPowheg, TH1F* hs_DYMadIncl, TH1F* hs_DYAmcIncl, TH1F* hs_data, TString xtitle, TString fname, Float_t minMll, Float_t maxMll, Float_t minSubleadLeptonPt, Float_t minLeadLeptonPt, Float_t maxLeptonEta, Int_t writeAction, std::string channel)
{
	...
	TString plotTitle = "CMS Preliminary Signal Region";
	plotTitle += xtitle;
	...
	TString cuts = "_minLeadLeptPt_" + to_string(minLeadLeptonPt) +"_minSubleadLeptPt_" + to_string(minSubleadLeptonPt) + "_minLeadJetPt_" + to_string(leadJetPtCut) + "_minSubleadJetPt_" + to_string(subLeadJetPtCut) + channel +"_signalRegion";
