...
process = cms.Process("AnalyzeDYJetsDecay")
...
process.load('ExoAnalysis.cmsWR.dyJetsGenAndRecoForNMinusOne_cff')
...
if (options.channel=='EE'):
	...
	matchedGenLeptonCollName = "matchedGenEleFromDY"
	recoLeptCollName = "heepRecoEle"
#
if (options.channel=='MuMu'):
	genLeptonCollName = "bareGenMu"
	matchedGenLeptonCollName = "matchedGenMuonFromDY"
...
process.analyzerTwo = process.analyzerOne.clone(
		...
		hadronCollection = cms.InputTag("matchedGenPartonFromDY"),
...
process.analyzerFour = process.analyzerTwo.clone(
		...
		inputHadronsAreQuarks = cms.bool(False),
		hadronCollection = cms.InputTag("condenseMatchedGenJetDYCollName"),
		)
...
if (options.channel=='EE'):
	...
	process.analyzeWRdecayGenQuarksWithMatching = cms.Path(process.matchedGenEleAndQuarkFromDYSeq*process.analyzerTwo)
	...
	process.analyzeWRdecayGenJetsWithMatching = cms.Path(
			process.matchedGenEleAndQuarkFromDYSeq
			*process.genJetSeq
			*process.matchedGenJetDYSeq
			*process.condenseGenJetDYSeq
...
if (options.channel=='MuMu'):
	...
	process.analyzeWRdecayGenJetsWithMatching = cms.Path(
			process.matchedGenMuAndQuarkFromDYSeq
			*process.genJetSeq
			*process.matchedGenJetDYSeq
			*process.condenseGenJetDYSeq
			*process.analyzerFour)
