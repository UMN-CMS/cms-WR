...
void quickDYTnPMiniPlotter(){
  ...
  TChain * chain_DY = new TChain("treeDyCheck","DYMAD");
  TChain * chain_ttbar = new TChain("treeDyCheck","NOTT");
#endif

  TChain * chain_WJets = new TChain("treeDyCheck","WJets");
  TChain * chain_WZ = new TChain("treeDyCheck","WZ");
  TChain * chain_ZZ = new TChain("treeDyCheck","ZZ");
...
#ifdef doOnlyDYMadInclAndHT
	dy = chain_DY->Add(localDir+"selected_tree_DYMAD_dytagandprobeEEInclusiveLowHTNoMllUpperBound.root");
	tt = chain_ttbar->Add(localDir+"selected_tree_DYMADHT_dytagandprobeEEOnlyHT100to200NoMllUpperBound.root");
	wjets = chain_WJets->Add(localDir+"selected_tree_DYMADHT_dytagandprobeEEOnlyHT200to400NoMllUpperBound.root");
    wz = chain_WZ->Add(localDir+"selected_tree_DYMADHT_dytagandprobeEEOnlyHT400to600NoMllUpperBound.root");
    zz = chain_ZZ->Add(localDir+"selected_tree_DYMADHT_dytagandprobeEEOnlyHT600toInfNoMllUpperBound.root");
#endif
...
#ifdef doOnlyDYMadInclAndHT
	dy = chain_DY->Add(localDir+"selected_tree_DYMAD_dytagandprobeMuMuInclusiveLowHTNoMllUpperBound.root");
	tt = chain_ttbar->Add(localDir+"selected_tree_DYMADHT_dytagandprobeMuMuOnlyHT100to200NoMllUpperBound.root");
	wjets = chain_WJets->Add(localDir+"selected_tree_DYMADHT_dytagandprobeMuMuOnlyHT200to400NoMllUpperBound.root");
    wz = chain_WZ->Add(localDir+"selected_tree_DYMADHT_dytagandprobeMuMuOnlyHT400to600NoMllUpperBound.root");
    zz = chain_ZZ->Add(localDir+"selected_tree_DYMADHT_dytagandprobeMuMuOnlyHT600toInfNoMllUpperBound.root");
#endif
...
void MakeHistos(TChain * chain, Selector *myEvent, std::vector<TH1F*> *hs){
  ...
  TH1F *h_WR_mass = new TH1F("h_WR_mass","",40,100,3000);
 
  TH1F *h_dilepton_mass = new TH1F("h_dilepton_mass","",50,60,1200);
  TH1F *h_nPV = new TH1F("h_nPV","",100,0,100);

  TH1F *h_Z_pt = new TH1F("h_Z_pt", "", 40, 0., 1200.);
  TH1F *h_WR_mass_lowZpt = new TH1F("h_WR_mass_lowZpt","",40,10,3000);
  ...
  for(int ev = 0; ev<nEntries; ++ev){
	...
	if(!separatedLeptonsAndJets(myEvent->lead_jet_pt, myEvent->sublead_jet_pt, myEvent->lead_jet_eta, myEvent->lead_jet_phi, myEvent->sublead_jet_eta, myEvent->sublead_jet_phi, myEvent->lead_lepton_eta, myEvent->lead_lepton_phi, myEvent->sublead_lepton_eta, myEvent->sublead_lepton_phi) ) continue;
...
void drawPlots(TH1F* hs_DY,TH1F* hs_ttbar,TH1F* hs_WJets,TH1F* hs_WZ,TH1F* hs_ZZ,TH1F* hs_data, TString xtitle, TString fname){
...
#ifdef doOnlyDYMadInclAndHT
  leg->AddEntry( hs_DY, "DYMadIncl HT<100" ) ; 
  leg->AddEntry( hs_ttbar, "DYMadHT100to200" ) ;
  leg->AddEntry( hs_WJets, "DYMadHT200to400" ) ; 
  ...
  */
  ...
  TString fn = fname;
#ifndef doOnlyDYMadInclAndHT
  if(channel == Selector::EE) fn += "_DYTnPRegion_NoMllUpperBound_EEChannelWR2p2TeVandBkgndMC_TTBarFromData_FixedBinWidthNoRatio";
  if(channel == Selector::MuMu) fn += "_DYTnPRegion_NoMllUpperBound_MuMuChannelWR2p2TeVandBkgndMC_TTBarFromData_FixedBinWidthNoRatio";
#endif
//Float_t leadJetPtCut = -10;
//Float_t subleadJetPtCut = -10;

  TString cuts = "_separatedLeptonsAndJets_minLeadLeptPt_35_minSubleadLeptPt_35_minLeadJetPt_"+ to_string(leadJetPtCut) +"_minSubleadJetPt_" + to_string(subleadJetPtCut);

#ifdef doOnlyDYMadInclAndHT
  if(channel == Selector::EE) fn += cuts + "_DYTnPRegion_NoMllUpperBound_OnlyDYMadInclAndDYMadHTBinned_EEChannel_FixedBinWidthNoRatio";
  if(channel == Selector::MuMu) fn += cuts + "_DYTnPRegion_NoMllUpperBound_OnlyDYMadInclAndDYMadHTBinned_MuMuChannel_FixedBinWidthNoRatio";
#endif
