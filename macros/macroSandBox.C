 ...
 *
 */
void makeAndSaveMultipleCurveOverlayHisto(map<string,TChain *> inputChainMap,TString canvName,Float_t legXmin,Float_t legYmin,Float_t legXmax,Float_t legYmax,Bool_t doNormalizationByArea,string title,string xLabel,string outputFileNameModifier,Bool_t specialGrouping,Float_t cutVal,TString firstLegEntry, bool doSpecialFxn){
	gStyle->SetOptStat("");  //no stats box
	...
	for(map<string,TChain*>::const_iterator chMapIt=inputChainMap.begin(); chMapIt!=inputChainMap.end(); chMapIt++){
		string oneHistoName = "";
		//std::cout<<"doSpecialFxn = "<< doSpecialFxn << std::endl;

		size_t openParenth = (chMapIt->first).find_first_of('('), lastChevron = ((chMapIt->first).find_first_of('>')+1);
		string uncutHistoName(chMapIt->first);
		///now initialize a new string, get rid of the content in uncutHistoName before '>>' and after '(',
		///and store the substring in the new string object
		oneHistoName = uncutHistoName.substr(lastChevron+1,openParenth-lastChevron-1);
		if(!doSpecialFxn){
	...
	//short colorsSpecial[] = {kBlack,kBlue,kRed};	//for fixed MWR 2200 and variable MNu
	...
	for(cMapIt mIt=overlayHistoMap.begin(); mIt!=overlayHistoMap.end(); mIt++){
		...
		if(doNormalizationByArea){
			...
			(mIt->second)->GetYaxis()->SetTitleOffset(1.15);
		}
	}///end loop over elements in overlayHistoMap
	
	string outputFile, outputFilePdf, outputFileC;
	...
	canv->SaveAs(outputFileC.c_str(),"recreate");
	//canv->Clear();
	canv->Close();
	delete canv;
	delete leg;
...
///end lowMassFlavorSidebandBkgndOnData
	...
	*/
	...
	//string histoEndings[] = {"_sqrtProdGenLeptMomentumMag(26,0.,1300.)"};	//several MWR, MNu is half MWR
...
*/
	...
	for(Int_t i=0; i<numMassWindows; i++){
		...
		if(i== 0) EEBkgndTwoForStack->SetBinContent(i+1, 39.71);
		if(i== 1) EEBkgndTwoForStack->SetBinContent(i+1, 6.09);
		if(i== 2) EEBkgndTwoForStack->SetBinContent(i+1, 0.911);
		...
		if(i== 0) EEBkgndTwoSystUnc->SetBinContent(i+1, 0.9299);
		if(i== 1) EEBkgndTwoSystUnc->SetBinContent(i+1, 0.2701);
		if(i== 2) EEBkgndTwoSystUnc->SetBinContent(i+1, 0.040);
		...
		if(i== 0) EEBkgndTwoStatUnc->SetBinContent(i+1, 3.387);
		if(i== 1) EEBkgndTwoStatUnc->SetBinContent(i+1, 0.762);
		if(i== 2) EEBkgndTwoStatUnc->SetBinContent(i+1, 0.153);
		...
		if(i== 0) MuMuBkgndTwoForStack->SetBinContent(i+1, 61.36);
		if(i== 1) MuMuBkgndTwoForStack->SetBinContent(i+1, 12.11);
		if(i== 2) MuMuBkgndTwoForStack->SetBinContent(i+1, 2.952);
		...
		if(i==0 ) MuMuBkgndTwoSystUnc->SetBinContent(i+1, 1.737);
		if(i==1 ) MuMuBkgndTwoSystUnc->SetBinContent(i+1, 0.566);
		if(i==2 ) MuMuBkgndTwoSystUnc->SetBinContent(i+1, 0.132);
		...
		if(i==0 ) MuMuBkgndTwoStatUnc->SetBinContent(i+1, 6.219);
		if(i==1 ) MuMuBkgndTwoStatUnc->SetBinContent(i+1, 1.265);
		if(i==2 ) MuMuBkgndTwoStatUnc->SetBinContent(i+1, 0.297);
	...
	legEE->AddEntry(EEBkgndTwoForStack, "DY MadHT+Incl");
	...
	legMuMu->AddEntry(MuMuBkgndTwoForStack, "DY MadHT+Incl");
