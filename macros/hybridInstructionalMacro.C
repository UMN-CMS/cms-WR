...
int main(){
	...
	///a RooWorkspace is a persisting container for RooFit projects
	///a RooWorkspace can contain and own variables, pdfs, fxns, and datasets
	///as well as classes which are not part of ROOT, and the source code (interface, src) for such classes
	RooWorkspace* w = new RooWorkspace("w");
	///c[num1,num2,num3] = varName[initial val, min val, max val]
	///Poisson distributions have a mean value, but no well defined std deviation or spread
	///to define a Poisson pdf I must specify two variables - one for the mean, and the other for the domain 
	///the first factory call defines a Poisson pdf named px, and a variable named x (observable) and a fxn splusb (expectations)
	///splusb is constructed from the sum of two variables - s (inital val = 0, domain = 0 to 100), and
	///b (initial val = 100, domain = 0 to 300)
	w->factory("Poisson::px(x[150,0,500],sum::splusb(s[0,0,100],b[100,0,300]))");
	
	///the second argument in Poisson::py() creates a fxn named taub which consists of
	///one variable tau, whose only value is 1., multiplied by a variable b, whose initial
	///val and domain are defined earlier
	w->factory("Poisson::py(y[100,0,500],prod::taub(tau[1.],b))");
	
	///PROD::model(px,py) creates a pdf by multiplying the Poisson pdf named px with the Poisson pdf named py
	///both px and py are defined earlier
	w->factory("PROD::model(px,py)");
	
	///Uniform::prior_b(b) defines a pdf named prior_b with a uniform distribution, normalized to 1, using
	///the variable b defined earlier (possible values are integers btwn 0 and 300)
	...
	///PROJ::averagedModel constructs a pdf from the convolution of a variable with a pdf
	///the variable is b (initial val = 100, domain btwn 0 and 300)
	///and the pdf is made from a product of three pdfs defined earlier
	///one pdf is px conditional on b, another is py, and the third is prior_b
	...
	/// RooPlot is a plot frame and container for graphics objects (histos, graphs, etc)
	/// RooPlot objects, much like TH1 objects, have public methods for setting range and labels
	/// in addition, RooPlot objects can hold histograms, curves representing a fit model, and txt labels
	...
	/// set the values of y and x which are used in the Poisson px and py pdfs
	/// create a cumulative probability function (integral of this fxn over entire
	/// domain = 1) from the "averagedModel" pdf and the variable x (observable)
	/// which is set to 150
	/// basically we want to calculate the p value associated with the observable x taking a value of 150
	/// in the averagedModel pdf
	/// once the p value is obtained, the significance (in sigma) can be calculated
	w->var("y")->setVal(100);
	w->var("x")->setVal(150);
	...
	/// since averagedModel, px, and py are all analytic pdfs, the numerical fit
	/// result (which gave a p value) from part 2 can be compared to an analytic result
	/// which is obtained here
	/// as with part 2, the question we are answering here is what is the p value associated
	/// with the observable x taking a value of 150, and the aux measurement yielding a value
	/// of y = 100? 
	...
	///define two variable sets x and s
	///x is the observable, s is the number of signal events
	w->defineSet("obs","x");
	w->defineSet("poi","s");
	...
	/// ModelConfig is a class within the RooStats namespace
	// create the null (background-only) ModelConfig with s=0
	/// w is a pointer to a RooWorkspace
