...
for MWR in MWRs:
	...
	for low_m, high_m in product(low_mass_bins, hi_mass_bins):
		...
		#original signal_tuple = ("WR_eejj", signal.Integral(low_bin, hi_bin) * (0.01/50000.) * lumi)
		...
		#in command defined below, the sequence "-S", "0" means no systematic effects are evaluated
		#in command defined below, the sequence "-n", datacard sets the job name equal to datacard
		command = ["combine","-M",method,"-S","0",datacard_file,"-n", datacard,"-t","1000"]
		#ret = combineTools.runCombine(command,identifier)
