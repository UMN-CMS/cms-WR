...
#use TTBar and topW MC for optimization, for better statistics
top = combineTools.getMassHistoTwo(True, treename, ("TT_"+identifier, "analysisCppOutputRootFiles/selected_tree_TT_flavoursidebandEMu.root"), binsize=binsize)
topUnwgt = combineTools.getMassHistoTwo(False, treename, ("UnweightedTT_"+identifier, "analysisCppOutputRootFiles/selected_tree_TT_flavoursidebandEMu.root"), binsize=binsize)
topW = combineTools.getMassHistoTwo(True, treename, ("top_TT_"+identifier, "analysisCppOutputRootFiles/selected_tree_topW_flavoursidebandEMuEE.root"), binsize=binsize)
topWUnwgt = combineTools.getMassHistoTwo(False, treename, ("Unweighted_top_TT_"+identifier, "analysisCppOutputRootFiles/selected_tree_topW_flavoursidebandEMuEE.root"), binsize=binsize)
...
for MWR in MWRs:
	...
	for low_m, high_m in product(low_mass_bins, hi_mass_bins):
		...
		bg_tuples = []
		if (top.Integral(low_bin, hi_bin)+ topW.Integral(low_bin, hi_bin) ) > 0: bg_tuples.append(("TTBar", top.Integral(low_bin, hi_bin) + topW.Integral(low_bin, hi_bin)))
		...
		###only for adding syst and stat uncertainties to datacard
		...
		#systematics.add("WR_"+identifier+"jj", "lumi", 1.027)
		...
		command = "combine -M %s -H ProfileLikelihood -S0 %s -n %s --toys %s" %(method, datacard_file, datacard, ntoys)
