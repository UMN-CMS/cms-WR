...
#queue=2nd
queue=1nw
tag=3200toysEleMuScaleSmearingSyst
...
#
###EMu ttbar mc with MuMu mass windows
bsub -q $queue -eo $logdir/ttbar_emu_mumucut_${tag}.err -oo $logdir/ttbar_emu_mumucut_${tag}.out "$PWD/runJob.sh EMu TT $finaldir $PWD \"$arguments  --cut_channel MuMu\""

###EMu ttbar mc with EE mass windows
bsub -q $queue -eo $logdir/ttbar_emu_eecut_${tag}.err -oo $logdir/ttbar_emu_eecut_${tag}.out "$PWD/runJob.sh EMu TT $finaldir $PWD \"$arguments  --cut_channel EE\""
