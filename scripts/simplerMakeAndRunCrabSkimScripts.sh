...
#pull the dataset path names and short names from a txt file
datasets=(`cat $datasetFile | grep -v '#' | awk '{print $2}'`)
datasetNames=(`cat $datasetFile | grep -v '#' | awk '{print $1}'`)

#make a tmp directory if one does not yet exist
...
case $datasetName in 
	DoubleEG*)
		params="$params, 'saveTnP=1', 'GT=74X_dataRun2_Prompt_v5'"
		LUMI=1000
		;;
	SingleMu*)
		params="$params, 'saveTnP=1', 'GT=74X_dataRun2_Prompt_v5'"
		...
		;;
	MuEG*)
		params="$params, 'saveTnP=0', 'GT=74X_dataRun2_Prompt_v5'"
		LUMI=1000
		;;
	DY*)
...
config.Data.outLFNDirBase = '/store/group/phys_exotica/leptonsPlusJets/WR/skims/'
