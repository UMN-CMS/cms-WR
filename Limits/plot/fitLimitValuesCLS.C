double findEstimate(TTree* tuple, double target, double llimit=0.05) {
  ...
  */
  ...
  const double* x=tuple->GetV2();
  const double* y=tuple->GetV1();

  for (int i=0; i<tuple->GetSelectedRows(); i++) {
    if (fabs(y[i]-0.5)<fabs(y[i]-yhalf)) {
      yhalf=y[i];
      xhalf=x[i];
    }
  ...
  TF1* f1=new TF1("exp","exp([1]*x+[0])",xhalf,2.5);
...
void fitLimitValue(TFile* file, double mw, double mn, bool batch=false) {
  ...
  if (c1!=0) c1->cd(1);
  tr->Draw("cls.exp:info.xsec",cut,"BOX");
  ...
  */
  ...
  if (c1!=0) c1->cd(2);
  tr->Draw("cls.exp_p1s:info.xsec",cut,"BOX");
  ...
  */
  ...
  if (c1!=0) c1->cd(3);
  tr->Draw("cls.exp_p2s:info.xsec",cut,"BOX");
...
void fitLimitValuesCLS(TFile* file, const char* ofname=0) {
  int pairs[100];
  int np=0;
  ...
  for (int i=0; i<nv; i++) {
    ...
    if (j==np) {
      ...
      fitLimitValue(file,mw,mnu,true);
