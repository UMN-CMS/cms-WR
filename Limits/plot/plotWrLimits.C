...
void plotLimits(int minval = -1, int maxval = -1, bool doMuon = true, std::string limitFile = "limitSummary.txt", std::string csFile = "cs.txt")
{
    ...
    for(int imwr = minval; imwr <= maxval; imwr += 100)
    {
        ...
        for(int imnu = 10; imnu < imwr; imnu += 1)
        {
            ...
            std::pair<int, int> masspoint(imwr, imnu);
            rawObsPts[masspoint] = eff_wr / acc;
        }
        ...
        for(int imnu = 10; imnu < imwr; imnu += 1)
        {
            ...
            std::pair<int, int> masspoint(imwr, imnu);
            rawExpPts[masspoint] = eff_wr / acc;
        }
    }
    ...
    for(int iwr = minval; iwr <= maxval; iwr += 100)
    {
        ...
        TLegend *leg = new TLegend(0.55, 0.72, 0.94, 0.94, leglabel, "brNDC");
    ...
    double max = 0.0;
    std::sort(obs_justExcluded.begin(), obs_justExcluded.end(), compareByNu2());
    std::sort(obs_justNotExcluded.begin(), obs_justNotExcluded.end(), compareByNu2());
    ...
    for(unsigned int i = 0; i < obs_justExcluded.size(); i += 5)
    {
        ...
        double x2 = obs_justNotExcluded.at(i).x;
        double y2 = obs_justNotExcluded.at(i).z;
        ...
        if(interpol > x1 && interpol < x2)
        {
            ...
            if(interpol > max) max = interpol;
        }
    }
    std::cout << "Max Observed MWR: " << max << std::endl;

    std::sort(exp_justExcluded.begin(), exp_justExcluded.end(), compareByNu2());
    std::sort(exp_justNotExcluded.begin(), exp_justNotExcluded.end(), compareByNu2());
    for(unsigned int i = 0; i < exp_justExcluded.size(); i += 5)
    {
        double x1 = exp_justExcluded.at(i).x;
        double y1 = exp_justExcluded.at(i).z;
        double x2 = exp_justNotExcluded.at(i).x;
        double y2 = exp_justNotExcluded.at(i).z;
        double interpol = -y1 * (x2 - x1) / (y2 - y1) + x1; // notice possible divide-by-zero!
